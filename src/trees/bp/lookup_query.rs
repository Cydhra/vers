...
const fn calculate_lookup_table(fwd: bool) -> [u64; 1 << LOOKUP_BLOCK_SIZE] {
    ...
    while v <= LOOKUP_MAX_VALUE {
        ...
        let mut encoded: u64 = ((minimum_excess as i32 + LOOKUP_BLOCK_SIZE as i32) as u64 & ENCODING_MASK) << MINIMUM_EXCESS_POSITION;
        ...
        while relative_off <= (LOOKUP_BLOCK_SIZE * 2) as usize {
            encoded |= ((query_map[relative_off] & 0b111) as u64) << (QUERY_BASE_POSITION + (relative_off * 3)) as u64;
...
const fn calculate_values_fwd(v: u32, minimum_excess: &mut SignedLookupBlockType, maximum_excess: &mut SignedLookupBlockType, query_map: &mut [i8; 17]) {
...
const fn calculate_values_bwd(v: u32, minimum_excess: &mut SignedLookupBlockType, maximum_excess: &mut SignedLookupBlockType, query_map: &mut [i8; 17]) {
    let mut total_excess = 0;
...
pub(crate) fn process_block_fwd(block: LookupBlockType, relative_excess: &mut i64) -> Result<u64, ()> {
    if *relative_excess <= lookup_maximum_excess(block) && lookup_minimum_excess(block) <= *relative_excess {
        Ok(answer_query(PAREN_BLOCK_LOOKUP_FWD[block as usize], *relative_excess))
    } else {
...
pub(crate) fn process_block_bwd(block: LookupBlockType, relative_excess: &mut i64) -> Result<u64, ()> {
    ...
    if (*relative_excess + total_excess == 0) || (lookup_minimum_excess(block)
        ...
        Ok(answer_query(PAREN_BLOCK_LOOKUP_BWD[block as usize], *relative_excess))
    } else {
        *relative_excess += total_excess;
        Err(())
    }
